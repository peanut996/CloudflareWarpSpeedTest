...
func InitHandshakePacket() {
	if ReservedString != "" {
		if PrivateKey == "" {
			log.Fatalln(i18n.QueryI18n(localizerWarping, "Reserved Empty"))
		}
		r, err := utils.ParseReservedString(ReservedString)
		if err != nil {
			log.Fatalln(i18n.QueryI18n(localizerWarping, "Reserved Invalid") + err.Error())
		}
		reserved = r
	}
	...
	if err != nil {
		log.Fatalln(i18n.QueryI18n(localizerWarping, "PrivateKey Invalid") + err.Error())
	}

	pub, err := getNoisePublicKeyFromBase64(PublicKey)
	if err != nil {
		log.Fatalln(i18n.QueryI18n(localizerWarping, "PublicKey Invalid") + err.Error())
	}
...
func buildHandshakePacket(pri device.NoisePrivateKey, pub device.NoisePublicKey) []byte {
	...
	if err != nil {
		log.Fatalln(i18n.QueryI18n(localizerWarping, "Build HandshakePacket Failed") + err.Error())
	}
...
func encodeBase64ToHex(key string) (string, error) {
	...
	if err != nil {
		return "", errors.New(i18n.QueryI18n(localizerWarping, "Base64 Invalid") + key)
	}
	if len(decoded) != 32 {
		return "", errors.New(i18n.QueryI18n(localizerWarping, "NoiseKey Invalid") + key)
	}
